MODULE matrices
    ...
    subroutine allocate_cmatrix(matrix, N, M, opt)
        ...
        if (present(opt) .and. opt .eqv. .TRUE.)then
            allocate(matrix%adj(N,M), stat= istat)
            if (istat/=0) then
                print*, "Error in allocating the matrix elements"
            end if
        end if
...
MODULE ising
    ...
    function tensor_product(A, B)result(M)
        ...
        call allocate_cmatrix(M, A%dim(1)*B%dim(1), A%dim(1)*B%dim(1))
        do ii=1, B%dim(1)
            do jj=1, B%dim(2)
                do kk=1, A%dim(1)
                    do qq=1, A%dim(2)
...
PROGRAM main
    ...
    type(cmatrix) :: pauli_x, pauli_z, H, temp
    double precision :: lambda
    integer :: N, ii, jj, istat, l_resolution

    ! Inputs!!!!!!!!
    print*, "Enter the number of qbits (INTEGER NUMBER): "
    read(*,*) N
    lambda = 0.
    l_resolution = 20
    ...
    do jj=1, l_resolution+1
        ...
        print*, "Iteration with N=", N, "lambda=", lambda
        ...
        do ii=1, N
            temp = field_interaction_op(pauli_z, ii, N)

            H%elem = H%elem + temp%elem
            call deallocate_cmatrix(temp)
        end do

        do ii=1, N-1
            temp = pair_interaction_op(pauli_x, ii, ii+1, N)

            H%elem = H%elem + lambda*temp%elem
            call deallocate_cmatrix(temp)
        end do
        ...
        ! write to text file
        write(50,*, iostat=istat) lambda, (H%eigenvals(ii), ii=1, 4)
        if (istat /= 0) then
        ...
        ! update lambda
        lambda = lambda + 3./(FLOAT(l_resolution))
